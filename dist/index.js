...
// ── Tools ────────────────────────────────────────────────
server.tool('search_products', 'Search for grocery products across Australian supermarkets. Returns product names, prices, and availability.', {
    ...
    if (store === 'woolworths' || store === 'both') {
        try {
            ...
            results.push({ name: `Woolworths search error: ${e.message}`, price: null, unit: '', stockcode: '', url: '', store: 'woolworths', available: false });
        }
    }
    if (store === 'coles' || store === 'both') {
        try {
            ...
            results.push({ name: `Coles search error: ${e.message}`, price: null, unit: '', productId: '', url: '', store: 'coles', available: false, brand: null });
        }
    }
    return {
        content: [{
...
server.tool('compare_prices', 'Compare prices for a list of grocery items across Woolworths and Coles. Returns the cheapest option for each item and the total savings.', {
    ...
    for (const item of items) {
        ...
        if (wPrice !== null && cPrice !== null)
            cheapest = wPrice <= cPrice ? 'woolworths' : 'coles';
        else if (wPrice !== null)
            cheapest = 'woolworths';
        else if (cPrice !== null)
            cheapest = 'coles';
        if (wPrice !== null)
            woolworthsTotal += wPrice;
        if (cPrice !== null)
            colesTotal += cPrice;
        comparisons.push({
            item,
...
server.tool('get_product_detail', 'Get detailed information about a specific Woolworths product including ingredients, nutrition, and pricing.', {
...
server.tool('get_categories', 'List all product categories available at Woolworths.', {}, async () => {
